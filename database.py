...
def _safe_int_or_log(value, name="value"):
    """Convert to int, log warning if fails, return None."""
    try:
        return int(value)
...
class DatabaseManager:
    def __init__(self, uri: str, db_name: str = "DiamondBotDB"):
        ...
        self.CACHE_TTL_VALID = 24 * 60 * 60
        self.CACHE_TTL_INVALID = 5 * 60
    ...
    async def increment_monthly_count(self, quantity, item_type: str = "dia"):
        ...
        try:
            ...
            logger.error(f"Error incrementing monthly count for {item_type}/{quantity_str}: {e}")
    ...
    async def check_license_local(self, user_id) -> Tuple[bool, Optional[datetime]]:
        ...
        if user_id is None:
            return False, None
        try:
            doc = await self.licenses.find_one({"user_id": user_id})
        except Exception as e:
    ...
    async def fetch_license_from_master(self, user_id) -> Tuple[bool, Optional[datetime]]:
        ...
        url = f"{base_url}/api/license/check/{user_id}"
        timeout = aiohttp.ClientTimeout(total=10)
        session = await self._get_session()
        headers = {"Authorization": f"Bearer {self.API_SECRET_TOKEN}"}
        try:
    ...
    async def _get_license_fresh(self, user_id) -> Tuple[bool, Optional[datetime]]:
        if self.is_client_mode:
    ...
    async def add_or_update_price(self, diamond, item_type: str = "dia") -> bool:
        ...
        try:
            await self.prices.update_one(
                ...
                {"$set": {"is_active": True}, "$setOnInsert": {"monthly_count": 0}},
                upsert=True
            )
            return True
        except Exception as e:
            logger.error(f"Error updating price {item_type}/{diamond_str}: {e}")
            return False
    ...
    async def get_price_by_amount(self, item_type: str, amount) -> Optional[dict]:
        ...
        if not amount_str:
            return None
        try:
            return await self.prices.find_one({"type": item_type, "diamond": amount_str})
        except Exception as e:
    ...
    async def set_price_active(self, item_type: str, amount, is_active: bool):
        ...
        try:
            ...
            logger.error(f"Error setting price active {item_type}/{amount_str}: {e}")

    async def get_price_msg_id(self, item_type: str, amount) -> Optional[int]:
        amount_str = str(amount) if amount is not None else ""
        if not amount_str:
            return None
        doc = await self.get_price_by_amount(item_type, amount_str)
    ...
    async def set_price_msg_id(self, item_type: str, amount, msg_id: int):
        ...
        try:
            ...
            logger.error(f"Error setting price msg_id for {item_type}/{amount_str}: {e}")
    ...
    async def get_service_status(self) -> bool:
        try:
            ...
            return False if setting and setting.get("status") == "Stop" else True
        except Exception as e:
    ...
    async def purge_3_months_old_data(self) -> Tuple[int, int]:
        ...
        try:
            res = await self.orders.delete_many(
                {"timestamps.created_at": {"$lt": three_months_ago}}
            )
            total_deleted_orders = res.deleted_count

            target_month_str = three_months_ago.strftime("%Y-%m")
            del_reports = await self.monthly_reports.delete_many(
                {"month": {"$lt": target_month_str}}
            )
            return total_deleted_orders, del_reports.deleted_count
        except Exception as e:
